<template>
  <div class="chat-header">
    ...
    <div class="chat-info">
      <h2>{{ headerTitle }}</h2>
      <span v-if="!isGroup && otherParticipant" class="participant-status">
...
const otherParticipant = computed(() => {
  ...
  return participants.value.find(p => p.username !== currentUsername) || null;
});

const headerTitle = computed(() => {
  if (isGroup.value) {
    return props.chat?.name || props.conversationPreview?.name || 'Group Chat';
  } else {
    return otherParticipant.value?.username || props.conversationPreview?.name || 'Private Chat';
  }
});
...
const headerImageAlt = computed(() => {
  if (isGroup.value) {
    ...
    return `${otherParticipant.value?.username || 'User'}'s Profile Picture`;
  }
});
